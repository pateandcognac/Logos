...
class Chora:
    ...
    def _start_camera_subscribers(self) -> None:
        ...
        Important: subscribing to PointCloud2 is expensive even if callbacks are light,
        ...
        with self._camera_subs_lock:
            ...
            # Freshen the frame event so we don't consume an old latched state
    ...
    def _pc2_to_xyz_numpy(self, pc_msg: PointCloud2) -> np.ndarray:
        ...
        # ros_numpy returns a structured array with named fields.
        # Most common is x,y,z. Sometimes padding exists.
        if arr.size == 0:
            return np.empty((0, 3), dtype=np.float64)

        # Extract fields to contiguous float64 vectors
        ...
        # Remove NaNs/Infs and non-positive depth early
    ...
    def _build_live_colored_pcd_map(
        ...
        # Fallback if ros_numpy unavailable or returned nothing (rare)
        if xyz.size == 0:
            xyz = np.array(
            ...
            # If a structured array slipped through
    ...
    def render(
        ...
        # try to reclaim large temporaries after render.
        try:
            import gc
            gc.collect()
        except Exception:
