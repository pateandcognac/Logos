...
def run():
    ...
    classifier_minutes = last_minutes_cfg.get('classifier', 5.0)

    # --- Speech transcript ---
    if speech_minutes == 0:
        print("Speech transcript muted. Set ambient_audio.last_minutes.speech to non-zero to enable.")
    else:
        transcripts = logos.sensory.get_ambient_transcript(last_minutes=speech_minutes, report=False)
        if transcripts:
            ...
            label = "last {:.4g}m".format(speech_minutes)
